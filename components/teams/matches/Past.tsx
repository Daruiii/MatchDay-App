            ...
            >
              ...
              <View style={styles.eventMatch}>
                ...
                {match?.winner?.slug?.includes(teamData?.teamName) ? (
                  <Text style={(styles.eventText as any)('#17B169')}>
                    {match?.results?.[0]?.score} - {match?.results?.[1]?.score}
                  </Text>
                ) : (
                  <Text style={(styles.eventText as any)('#960018')}>
                    {match?.results?.[0]?.score} - {match?.results?.[1]?.score}
                  </Text>
                )}
              ...
              <View style={styles.eventReplay}>
                <Pressable
                  onPress={() =>
                    router.push(
                      `https://www.youtube.com/results?search_query=${match?.serie?.slug}+${match?.opponents[1]?.opponent?.acronym}+vs+${match?.opponents[0]?.opponent?.acronym}`
                    )
                  }
                >
                  <Image
